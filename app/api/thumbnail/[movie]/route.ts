...
export async function GET(
  ...
  if (fs.existsSync(thumbnailPath)) {
    ...
    if (thumbnailRange) {
      ...
      return new NextResponse(Readable.toWeb(thumbnailFile) as ReadableStream, { status: 206, headers: head })
    } else {
      const head = {
      ...
      return new NextResponse(Readable.toWeb(thumbnailFile) as ReadableStream, { status: 200, headers: head })
    }
  } else {
    return new NextResponse(thumbnailPath , { status: 403 });
  }
}